...
class EntityMatch(object):
    ...
    def char_index(self, p_sentences, h_sentences):
        ...
        for p_sentence, h_sentence in zip(p_sentences, h_sentences):
            p = [
                self.word2idx[word.lower()] for word in p_sentence if
                len(word.strip()) > 0 and word.lower() in self.word2idx.keys()
            ]
            h = [
                self.word2idx[word.lower()] for word in h_sentence if
                len(word.strip()) > 0 and word.lower() in self.word2idx.keys()
            ]
            p_list.append(p)
            h_list.append(h)
    ...
    def predict(self, query):
        ...
        match_score = sorted(match_score.items(),
                             key=lambda x: x[1],
                             reverse=True)
...
if __name__ == '__main__':
    emm = EntityMatch("./data/code.txt")
    test = pd.read_csv("./data/test.csv")
    total = len(test)
    correct = 0
    for raw, norm in tqdm(test[["sentence1", "sentence2"]].values):
