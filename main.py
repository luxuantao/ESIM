...
class Config:
    def __init__(self):
        ...
        self.train_data_path = 'data/train_20200228.csv'
        self.dev_data_path = 'data/dev_20200228.csv'
        self.epochs = 20
...
class ESIM(nn.Module):
    ...
    def forward(self, bert_inputs_01, bert_inputs_02):
        ...
        attention_mask_01 = attention_mask_01.unsqueeze(2).repeat(1, 1, seq_len)  # batch size, seq len, seq len 

        sequence_output_02 = self.bert(**bert_inputs_02)
        sequence_heatmap_02 = sequence_output_02.last_hidden_state  # batch size, seq len, hid_dim
        attention_mask_02 = bert_inputs_02['attention_mask']
        attention_mask_02 = attention_mask_02.unsqueeze(1).repeat(1, seq_len, 1)  # batch size, seq len, seq len 
        sequence_heatmap_02_T = sequence_heatmap_02.permute(0, 2, 1)  # batch size, hid_dim, seq len

        d_k = sequence_heatmap_01.shape[-1]
        scores = torch.matmul(sequence_heatmap_01, sequence_heatmap_02_T) / math.sqrt(d_k)  # batch size, seq len, seq len

        mask = attention_mask_01 * attention_mask_02  # batch size, seq len, seq len 
        ...
        _sequence_heatmap_01 = torch.matmul(scores_01, sequence_heatmap_02)  # batch size, seq len, hid_dim
        _sequence_heatmap_02 = torch.matmul(scores_02.permute(0, 2, 1), sequence_heatmap_01)  # batch size, seq len, hid_dim
...
def test(model, dev_iter, batch):
    ...
    total_count = 0
    acc_count = 0
    for step, i in enumerate(tqdm(dev_iter)):
        bert_inputs_01, bert_inputs_02, labels = batch(i)
...
if __name__ == '__main__':
    ...
    test_data = EsimDataset('data/dev_20200228.csv')
